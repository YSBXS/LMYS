注意：频道名可以多个（20个大概4秒左右返回） 但是频道多会导致限流 所以还是少点好 （限流一次就是多等8秒左右）
新服务器方式仅支持20210929及以后的包

运行方式 不同平台替换以下tgsou-x86_64文件名
1.直接运行
./tgsou-x86_64
2.指定代理运行
./tgsou-x86_64 --proxy http://127.0.0.1:1084
第一次运行需要授权 输入+86再加上手机号（比如：+8618888888888），在tg客户端接受验证码 填入即可 后面可以后台运行
如果以前生成过这授权文件 直接放同一目录运行即可
3.后台运行
  nohup ./tgsou-x86_64 > my_log_file.log 2>&1 &
4.访问
